...
SCAN = posixpath.join(BASE, "cdft_Co48_adsorbate_fragment_charge_scan")
CASES = (
    ("OH_frag_Nm1", "al16_valid_co48_OH", (48, 221, 222), 24.184),
    ("OH_frag_Np1", "al16_valid_co48_OH", (48, 221, 222), 26.184),
    ("O_frag_Nm1", "al16_valid_co48_O", (48, 221), 23.342),
    ("O_frag_Np1", "al16_valid_co48_O", (48, 221), 25.342),
    ("OOH_frag_Nm1", "al16_valid_co48_OOH", (48, 221, 222, 223), 29.906),
    ("OOH_frag_Np1", "al16_valid_co48_OOH", (48, 221, 222, 223), 31.906),
)
...
def make_input(baseline: str, name: str, atoms: tuple[int, ...], target: float) -> str:
    value = re.sub(
        r"(?im)^\s*PROJECT_NAME\s+\S+",
        f"  PROJECT_NAME cdft_{name}",
        baseline,
        count=1,
    )
    value = re.sub(
...
def main() -> None:
    ...
    try:
        ...
        if "CoAds_cdft_q" in queue:
            raise RuntimeError("Co+adsorbate fragment scan is already active")
        mkdir(sftp, SCAN)
        manifest = ["task\tcase\tconstraint\ttarget\tatoms\tstatus"]
        case_names: list[str] = []
        for task, (name, source_name, atoms, target) in enumerate(CASES):
            case_names.append(name)
            dest = posixpath.join(SCAN, name)
            mkdir(sftp, dest)
            try:
                if sftp.stat(posixpath.join(ssh_ncw.REMOTE_ROOT, dest, "scan.out")).st_size:
                    raise RuntimeError(f"refusing to overwrite existing output for {name}")
            except FileNotFoundError:
                pass
            ...
            write_text(
                ...
                make_input(baseline, name, atoms, target),
            )
...
"""
        ...
        row = (
            f"2026-08-15\tfragment_scan_submission\t{job_id}_0-5\tpending\t"
            "OH_O_OOH_Co48_plus_adsorbate_charge_N0pm1;fixed_geometry;"
            "array_concurrency1;no_duplicates"
        )
        if row not in sched:
