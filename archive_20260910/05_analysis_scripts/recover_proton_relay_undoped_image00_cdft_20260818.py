...
SOURCE = posixpath.join(BASE, "undoped_image_00")
DEST = posixpath.join(BASE, "undoped_image_00_convergence_recovery")
SOURCE_PROJECT = "relay_pcet_undoped_00_Nm1"
RECOVERY_PROJECT = "relay_pcet_undoped_00_Nm1_recovery"
JOB_NAME = "RelayUndop00Nm1Recover"
TASK = 3
...
def extend_cdft_outer_scf(value: str) -> str:
    match = re.search(r"(?is)(&CDFT\b.*?&END\s+CDFT)", value)
    if not match:
        raise RuntimeError("missing CDFT block")
    block = match.group(1)
    replaced, count = re.subn(
        r"(?im)^(\s*MAX_SCF\s+)40\s*$", r"\g<1>80", block
    )
    if count != 1:
        raise RuntimeError(f"expected one CDFT MAX_SCF 40, found {count}")
    return value[: match.start()] + replaced + value[match.end() :]
...
def main() -> None:
    ...
    try:
        ...
        if JOB_NAME in queue:
            raise RuntimeError("convergence recovery already active")

        output = read_text(sftp, posixpath.join(SOURCE, "scan.out"))
        required = (
            "CDFT SCF loop FAILED to converge",
            "PROGRAM ENDED AT",
            "ABORT in dbcsr_lib.F:322",
        )
        if not all(marker in output for marker in required):
            raise RuntimeError("source is not the diagnosed cDFT failure")
        if "CDFT SCF loop converged" in output:
            raise RuntimeError("source unexpectedly contains a converged cDFT loop")
        ...
        value = extend_cdft_outer_scf(value)
...
"""
        ...
        fields[7] = (
            f"rejected_111141_{TASK}_CDFT_nonconverged_"
        ...
        scheduler += (
            f"2026-08-18\tCDFT_nonconvergence\t111141_{TASK}\trejected_scientific\t"
            f"target{target:.6f};final{current:.9f};strength{strength:.12f};"
            f"outer_MAX_SCF40_exhausted;DBCSR_cleanup_abort\n"
            f"2026-08-18\tconvergence_recovery_submission\t{job_id}\tpending\t"
            f"undoped_image00;latest_WFN;initial_strength{strength:.12f};"
            f"CDFT_outer_MAX_SCF80;n40;no_duplicate\n"
        )
