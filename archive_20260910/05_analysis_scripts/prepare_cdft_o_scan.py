...
SCAN = posixpath.join(BASE, "cdft_O_Co48_scan")
CASES = (
    ("charge_Nm1", "CHARGE", 17.470),
    ("charge_Np1", "CHARGE", 19.470),
    ("mag_Mm2", "MAGNETIZATION", 0.776),
    ("mag_Mp2", "MAGNETIZATION", 4.776),
)
...
def input_text(baseline: str, name: str, constraint: str, target: float) -> str:
    text = re.sub(r"(?im)^\s*PROJECT_NAME\s+\S+", f"  PROJECT_NAME cdft_O_Co48_{name}", baseline, count=1)
    ...
    cdft = f"""      &CDFT
      ...
      &END CDFT
"""
    return re.sub(r"(?im)^(\s*&END\s+QS\b)", cdft + r"\1", text, count=1)
...
ulimit -s unlimited
CASES=(charge_Nm1 charge_Np1 mag_Mm2 mag_Mp2)
