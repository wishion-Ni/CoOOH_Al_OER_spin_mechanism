...
static Header read_header(std::ifstream &input) {
    ...
    throw std::runtime_error("CHGCAR grid line not found");
}
...
int main(int argc, char **argv) {
    ...
    const char magic[8] = {'C','H','G','D','I','F','F','1'};
    ...
    for (std::int64_t index = 0; index < points; ++index) {
        if (!(oxygen >> oxygen_value) || !(hydroxyl >> hydroxyl_value))
            throw std::runtime_error("charge grid ended early");
    ...
    std::cout << std::setprecision(12)
              ...
              << "volume_A3=" << volume << '\n'
              << "integrated_delta_e=" << sum_raw / static_cast<double>(points) << '\n'
              << "density_min_e_A3=" << minimum << '\n'
              << "density_max_e_A3=" << maximum << '\n'
