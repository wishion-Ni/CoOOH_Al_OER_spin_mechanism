...
def main() -> None:
    ...
    try:
        ...
        accounting = run(client, f"sacct -j {args.job} --format=JobIDRaw,State,ExitCode -n -P")
        if f"{args.job}|COMPLETED|0:0" not in accounting:
            raise RuntimeError(f"job is not a clean Slurm completion: {accounting}")
        output = read_text(sftp, posixpath.join(BASE, args.case, "scan.out"))
        ...
        if "ABORT" in output or "SCF run NOT converged" in output or "CDFT SCF loop FAILED" in output:
            raise RuntimeError("scan contains a fatal or unconverged signature")
        ...
        row = (
            f"2026-08-16\tfragment_scan_completion\t{args.array_task}\tvalid\t"
            f"physical_job{args.job};{args.case}_target{target:.6f};final{current:.6f};"
