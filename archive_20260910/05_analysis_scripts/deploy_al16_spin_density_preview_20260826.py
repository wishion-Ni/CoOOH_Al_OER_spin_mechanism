...
CPP = ROOT / "artifacts/chgcar_magnetization_difference_downsample.cpp"
REMOTE_BASE = (
...
REMOTE_WORK = posixpath.join(
    REMOTE_BASE, "reaction_density_oh_to_o_same_geometry_20260823/Al16_spin_preview"
)
...
def main() -> None:
    ...
    try:
        ...
        with client.open_sftp() as sftp:
            ...
            slurm = f"""#!/bin/bash
#SBATCH --job-name=Al16_spin_rho
#SBATCH --partition=n28
#SBATCH --nodes=1
#SBATCH --ntasks=1
#SBATCH --cpus-per-task=1
#SBATCH --time=02:00:00
#SBATCH --output=spin_preview_%j.stdout
#SBATCH --error=spin_preview_%j.stderr
set -euo pipefail
cd {ssh.REMOTE_ROOT}/{REMOTE_WORK}
g++ -O3 -std=c++11 {CPP.name} -o chgcar_magnetization_difference_downsample
O=../Al16_adjacent/O_at_OHgeom/CHGCAR
OH=../../cohp_oh_o_corrected_20260820/Al16_adjacent/OH/CHGCAR
test -s "$O"
test -s "$OH"
./chgcar_magnetization_difference_downsample "$O" "$OH" Al16_O_minus_OH_spin_ds2.raw 2 | tee Al16_O_minus_OH_spin_ds2.stats
"""
            with sftp.file(posixpath.join(ssh.REMOTE_ROOT, REMOTE_WORK, "run_spin_preview.slurm"), "w") as handle:
                handle.write(slurm)

        command = f"cd {ssh.REMOTE_ROOT}/{REMOTE_WORK} && sbatch --parsable run_spin_preview.slurm"
        _, stdout, stderr = client.exec_command(command)
