...
SOURCE = posixpath.join(BASE, "undoped_image_00_convergence_recovery")
DEST = posixpath.join(BASE, "undoped_image_00_tight_scf_recovery")
SOURCE_PROJECT = "relay_pcet_undoped_00_Nm1_recovery"
RECOVERY_PROJECT = "relay_pcet_undoped_00_Nm1_tight_scf"
JOB_NAME = "RelayUndop00Nm1Tight"
TASK = 3
...
def tighten_electronic_scf(value: str) -> str:
    ...
    if count != 2:
        raise RuntimeError(f"expected two electronic EPS_SCF entries, found {count}")
...
def main() -> None:
    ...
    try:
        ...
        if JOB_NAME in queue:
            raise RuntimeError("tight-SCF recovery already active")

        output = read_text(sftp, posixpath.join(SOURCE, "recovery.out"))
        if "CDFT SCF loop FAILED to converge after   81 iterations" not in output:
            raise RuntimeError("source is not the diagnosed MAX_SCF80 failure")
        if "CDFT SCF loop converged" in output:
            raise RuntimeError("source unexpectedly contains cDFT convergence")
        if "PROGRAM ENDED AT" not in output or "ABORT in dbcsr_lib.F:322" not in output:
            raise RuntimeError("source ending does not match diagnosed failure")
        ...
        if abs(current - target) <= 1.5e-4:
            raise RuntimeError("source is already within accepted target tolerance")
        ...
        if sftp.stat(posixpath.join(ssh_ncw.REMOTE_ROOT, wfn)).st_size < 40_000_000:
            raise RuntimeError("latest restart WFN is too small")

        mkdir(sftp, DEST)
        for name in ("recovery.out", "recovery.err"):
            try:
                if sftp.stat(posixpath.join(ssh_ncw.REMOTE_ROOT, DEST, name)).st_size:
                    raise RuntimeError("tight-SCF recovery destination already has output")
            except FileNotFoundError:
                pass

        value = read_text(sftp, posixpath.join(SOURCE, "recovery.inp"))
        value = re.sub(
        ...
        value = tighten_electronic_scf(value)
...
"""
        ...
        expected = "rejected_111141_3_CDFT_nonconverged_recovery_111149_pending"
        ...
        fields[7] = (
            ...
            f"tight_SCF_recovery_{job_id}_pending"
        )
        ...
        scheduler += (
            f"2026-08-18\tCDFT_nonconvergence\t111149\trejected_scientific\t"
            f"target{target:.6f};final{current:.9f};strength{strength:.12f};"
            "CDFT_outer_MAX_SCF80_exhausted;DBCSR_cleanup_abort\n"
            f"2026-08-18\ttight_SCF_recovery_submission\t{job_id}\tpending\t"
            f"undoped_image00;latest_WFN;initial_strength{strength:.12f};"
            "electronic_EPS_SCF1E-7;CDFT_target_unchanged;n40;no_duplicate\n"
        )
