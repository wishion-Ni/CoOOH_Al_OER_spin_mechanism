...
CASES = {
    "undoped_OH": REMOTE_BASE / "undoped_control/OH_pair_corrected_nomadelung",
    "undoped_O": REMOTE_BASE / "undoped_control/O_nomadelung",
    "Al16_OH": REMOTE_BASE / "Al16_adjacent/OH_nomadelung",
    "Al16_O": REMOTE_BASE / "Al16_adjacent/O_nomadelung",
}
...
def main() -> None:
    ...
    target = Path(__file__).resolve().parent / "cohp_oh_o_inputs_20260824"
    ...
    try:
        ...
        for name, remote_dir in CASES.items():
            ...
            for filename in ("COHPCAR.lobster", "ICOHPLIST.lobster", "lobsterout", "POSCAR"):
                remote = str(remote_dir / filename)
                local = case_dir / filename
                sftp.get(remote, str(local))
                print(f"{name}: {filename} ({local.stat().st_size} bytes)")
    finally:
        client.close()
