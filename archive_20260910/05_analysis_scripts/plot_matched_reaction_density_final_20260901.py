...
DATA = ROOT / "reaction_density_final_20260901"
OUTPUT = ROOT / "reaction_density_undoped_vs_Al16_final_20260901.png"
...
def draw_framework(ax, projected):
    ...
    for first, second in zip(labels[:-1], labels[1:]):
        ...
        ax.plot([p1[0], p2[0]], [p1[1], p2[1]], color="#222222", lw=0.9, alpha=0.75)
...
def main():
    ...
    for case_name, case in CASES.items():
        charge, charge_lattice, charge_volume, charge_ds = read_density(
            DATA / f"{case_name}_O_minus_OH_charge_ds2.raw", b"CHGDIFF1"
        )
        spin, spin_lattice, spin_volume, spin_ds = read_density(
            DATA / f"{case_name}_O_minus_OH_spin_ds2.raw", b"MAGDIFF1"
        )
    ...
    u_min, v_min = uv.min(axis=0) - np.array([1.20, 1.10])
    u_max, v_max = uv.max(axis=0) + np.array([1.20, 1.10])
    u = np.linspace(u_min, u_max, 520)
    v = np.linspace(v_min, v_max, 340)

    for item in prepared.values():
        uu, vv, charge_plane = sample_plane(
            item["charge"], item["lattice"], item["active_o"], plane_center, axis_u, axis_v, u, v
        )
        _, _, spin_plane = sample_plane(
            item["spin"], item["lattice"], item["active_o"], plane_center, axis_u, axis_v, u, v
        )
        item["fields"] = [
            charge_plane,
            spin_plane,
    ...
    fig, axes = plt.subplots(2, 4, figsize=(15.5, 6.2), dpi=180, sharex=True, sharey=True)
    ...
    for row, case_name in enumerate(case_order):
        ...
        for column, (field, limit, cmap, title, unit) in enumerate(
            ...
            if row == 0:
                ax.set_title(title, fontsize=10.1, pad=7)
            if column == 0:
                ax.set_ylabel(
                    item["case"]["label"]
                    + rf"\n$\int\Delta m\,dV$ = {item['spin_integral']:+.3f} $\mu_B$"
                    + "\nIn-plane coordinate (A)",
                    fontsize=8.8,
                )
            if row == 1:
                ax.set_xlabel("In-plane coordinate (A)", fontsize=8.5)
            ax.set_aspect("equal")
            ...
            colorbar.set_label(unit, fontsize=7.3)

    fig.suptitle(
        r"Self-consistent reaction-induced density: $*\mathrm{OH}\rightarrow *\mathrm{O}$ at fixed $*\mathrm{OH}$ geometry",
        fontsize=11.5,
        y=1.01,
    )
    fig.text(
        0.5,
        -0.025,
        r"$\Delta\rho=\rho(*\mathrm{O}@*\mathrm{OH\ geom})-\rho(*\mathrm{OH}@*\mathrm{OH\ geom})$. "
        r"Identical plane, limits and color scales; each charge difference integrates to $-1.000\ e$. "
        r"Red/purple: positive response; blue/orange: negative response.",
        ha="center",
        fontsize=8.1,
    )
    fig.tight_layout(w_pad=0.75, h_pad=0.65)
    ...
    for case_name in case_order:
        ...
        for title, field in zip(titles, prepared[case_name]["fields"]):
            print(f"{case_name}\t{title}\tmin={field.min():.8f}\tmax={field.max():.8f}")
